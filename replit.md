...
2. **Run the seed** with the **service role** (RLS would otherwise block the inserts). The seed lives at `supabase/seed/seed.sql` and is generated by `supabase/seed/generate-seed.mjs` from the same deterministic mock data the in-memory preview uses. It populates one squadron (`00000000-0000-0000-0000-000000000001`) with 16 pilots, 80 sorties, 240 currency rows, leaves, duty roster, NOTAMs, and today's flight schedule. Re-running is idempotent (stable-key tables upsert; UUID-keyed and date-keyed tables are wiped per-squadron before reinsert).
3. **Create an admin auth user** in Supabase Auth, set `app_metadata` to `{"squadron_id": "00000000-0000-0000-0000-000000000001", "role": "admin"}`, then run the `insert into users (...)` statement at the bottom of `seed.sql`.
4. **Set client env vars** `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY` so `isLive()` flips on.
5. **Activate the license** in the desktop app with the seeded key `DEMO-RJAF-1234-5678`.
