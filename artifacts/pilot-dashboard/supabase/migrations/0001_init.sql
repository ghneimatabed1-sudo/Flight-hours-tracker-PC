...
create table if not exists pilots (
  id text primary key,
  squadron_id uuid not null default auth.squadron_id() references squadrons(id) on delete cascade,
  rank text not null,
  name text not null,
  arabic_name text,
  unit text,
...
create table if not exists audit_log (
  ...
  squadron_id uuid default auth.squadron_id() references squadrons(id) on delete cascade,
  type text not null,
  actor text,
...
create policy sq_select on squadrons
  for select using (id = auth.squadron_id());

drop policy if exists lic_select on licenses;
create policy lic_select on licenses
  for select using (squadron_id = auth.squadron_id());
...
create policy audit_insert on audit_log
  for insert with check (squadron_id is null or squadron_id = auth.squadron_id());
