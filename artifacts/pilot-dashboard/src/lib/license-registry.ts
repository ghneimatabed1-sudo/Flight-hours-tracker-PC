...
//
// LAN-only build: the v1.1.53 cloud `license_registry` mirror is gone
// along with the rest of the Supabase SDK. Every PC's super-admin
// keeps an authoritative copy in localStorage, and key portability
// across host PCs is handled by the operator runbook (export/import
// backup) rather than by an automatic cloud round-trip. The mirror
// helpers below are kept as no-ops so existing call sites continue to
// compile without conditional checks.
...
// ─── Cloud mirror — removed in LAN-only build ──────────────────────────
// The Supabase `license_registry` table is gone. The function below
// is kept as a no-op so the super-admin login flow can call it without
// conditionals; it returns immediately and never touches the network.
