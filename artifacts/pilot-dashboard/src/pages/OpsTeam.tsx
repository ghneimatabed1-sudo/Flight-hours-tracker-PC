...
import { UserPlus, Trash2, Copy, Check, ShieldAlert } from "lucide-react";
import { useI18n } from "@/lib/i18n";
import { PageHead } from "@/components/Layout";
import { useAuth } from "@/lib/auth";
import {
  listCommanders, createCommander, deleteCommander,
        ...
        <section className="bg-card border border-border rounded-lg p-4">
          ...
          {assigned.length === 0 ? (
            ...
            <ul className="divide-y divide-border">
              {assigned.map(c => (
                <li key={c.id} className="flex items-center justify-between py-2 text-sm" data-testid={`row-opsteam-${c.username}`}>
                  <div>
                    <div className="font-medium">{c.displayName}</div>
                    <div className="text-xs text-muted-foreground font-mono">{c.username}</div>
                  </div>
